"""
This file is basically a helper file for the api file:
app.py
"""
import os
...
def open_file(path, mode):
    try:
        ...
        return jsonify({'request': 'transactions', 'status': 'failed',
                        'result': 'error opening tmp file saved on server '
                        + 'side'}), 404


# whenever invalid data is encountered we want to reset
...
def validate_file(request_files, key, filepath):
    ...
    if request_files.get(key, 0) and (len(request_files.getlist(key)) == 1):
        ...
        # valid file name
        if filename_prefix:
            # check if csv file by checking last element in
            # list after a split by '.'
            ...
            if filename_list[-1].lower() == 'csv':
                ...
                # empty file
                    ...
                    remove_file(filepath)
                    return jsonify({'request': 'transactions',
                                    'status': 'failed',
                                    'result': 'input file is empty'}), 404

            # not a csv file file
            else:
                return jsonify({'request': 'transactions',
                                'status': 'failed',
                                'result': 'incorrect file extension, '
                                + 'ensure csv file is submitted'}), 415
        # empty file name (special chars/spaces only)
        else:
            return jsonify({'request': 'transactions',
                            'status': 'failed',
                            'result': 'filename is empty, dont use '
                            + 'special characters/only spaces when '
                            + 'naming your csv file" '}), 400
    # incorrect CURL syntax (e.g.: too many files sent with POST)
    else:
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'file not submitted correctly, '
                        + 'use the following syntax: curl -v -X POST -k '
                        + 'https://127.0.0.1/api/transactions -F '
                        + '"data=@data.csv" '}), 400
...
def process_csv_row(line, file_open, key, filepath):
    ...
    # empty column cases col A-D (0-3) isnt filled,
    # but E-Z might be (missing column entries case)
    if 0 in empty_col or 1 in empty_col or 2 in empty_col or 3 in empty_col:
        file_cleanup(file_open, key, filepath)
        reset_sums()
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'columns A-D are not filled '
                        + 'properly on row ' +
                        str(current_app.config.get('entries', 0)+1)}), 422
    ...
    # too many row entries
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'too many column entries on row ' +
                        str(current_app.config.get('entries', 0) + 1)}), 422


# function to check date field has been filled correctly
# (YYYY-mm-dd) and all of the same year
def verify_date(date, file_open, year_list, key, filepath):
    ...
    try:
        date_format = bool(datetime.strptime(date, formater))
    except ValueError:
        date_format = False
    if not date_format:
        file_cleanup(file_open, key, filepath)
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'incorrectly formatted date, '
                        + 'format as yyyy-mm-dd'}), 422
    ...
    # entry not of the same year as the rest of the entries
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'Please ensure all entries are from '
                        + 'the same year'}), 422
...
def process_type(val_type, file_open, key, filepath):
    if val_type == 'expense':
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'incorrectly formatted type, please '
                        + 'specify either expense or income'}), 422


# function to process memo (check if not all numbers)
def process_memo(memo, file_open, key, filepath):
    # use ascii to check if theres at least 1 letter in this
    # string (97 - 122 ;a-z)
    valid_memo = False
    for char in memo:
    ...
    if not valid_memo:
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'your memo does not have any english '
                        + 'characters (a-z), which does '
                        + 'not make sense'}), 422
    else:
        return None


# function to verify amount provided is: valid float,
# <=32 bits, < 2 decimal places
def process_amount(amount, file_open, key, filepath):
    # assumptions: no $ symbol infront of the values in csv file,
    # no commas present in the amount field (e.g.: 1,23),
    # scientific notation not valid
    ...
    if 'e' in amount:
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'your amount is not formatted properly. '
                        + 'Please dont use scientific notation'}), 422
    ...
    if invalid_amount:
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'your amount is not formatted properly. '
                        + 'please ensure to put just the numerical value '
                        + '(e.g.: 50.2 or 50 or 50.79) with no $ '
                        + 'preceeding the value'}), 422
    ...
    if abs(float_amount) > 0xffffffff:
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'Please input a realistic value in the '
                        + 'amount field (>32 bit number is not realistic '
                        + 'for your income or expense)'}), 422

    # confirm amount is limited to at most 2 decimal places
    # (51.32 makes sense, 51.323 doesnt)
    if len(str(amount).rsplit('.', maxsplit=1)[-1]) > 2:
        ...
        return jsonify({'request': 'transactions',
                        'status': 'failed',
                        'result': 'your amount has more than 2 decimal '
                        + 'places which is not a real life money value. '
                        + 'Please format to 2 decimal places or less '
                        + '(avoid scientific notation)'}), 422
...
def update_result(net, amount):
    # expenses
    if net == -1:
        current_app.config['expenses'] = \
            Decimal(str(current_app.config.get('expenses', 0))) \
            + Decimal(str(abs(amount)))
    # income
    elif net == 1:
        current_app.config['grossRevenue'] = \
            Decimal(str(current_app.config.get('grossRevenue', 0))) \
            + Decimal(str(abs(amount)))
...
# func to calculate net revenue, set calculated state to True,
# and return HTTP 200 code
def calculate_net_revenue(file_open, key, filepath):
    file_cleanup(file_open, key, filepath)
    current_app.config['netRevenue'] = \
        current_app.config.get('grossRevenue', 0) \
        - current_app.config.get('expenses', 0)
    current_app.config['calculated'] = True
    return jsonify({'request': 'transactions',
                    'status': 'success',
                    'result': 'data has been processed, send a GET '
                    + 'request to the /report endpoint to retrieve '
                    + 'the results'}), 200
